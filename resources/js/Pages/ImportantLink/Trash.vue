...
const items = reactive(props.important_links);
                ...
                <div class="bg-white dark:bg-slate-800 overflow-hidden shadow sm:rounded p-2">
                    <div class="flex justify-between">
                        <div class="flex shrink-0 rounded overflow-hidden">
                            ...
                            <RestoreBulk v-if="itemsSelected.length != 0 && can(['important link delete'])"
                                :itemsSelected="itemsSelected" title="Items" />
                            <DeleteForceBulk v-if="itemsSelected.length != 0 && can(['important link delete'])"
                                :itemsSelected="itemsSelected" title="Items" />
                        </div>
                    ...
                    <EasyDataTable class="mt-2" :rows-per-page="rowsPerPage" show-index alternating border-cell
                        ...
                        <template #item-actions="item">
                            <div class="flex w-fit rounded overflow-hidden">
                                <Restore v-show="can(['important link update'])" :title="item.name" :item="item"
                                    @open="item = item" />
                                <DeleteForce v-show="can(['important link delete'])" :title="item.name" :item="item"
                                    @open="item = item" />
                            </div>
                        </template>
                    </EasyDataTable>
