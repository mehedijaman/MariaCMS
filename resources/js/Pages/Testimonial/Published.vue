<script setup>
import Create from "@/Pages/Testimonial/Create.vue";
import Edit from "@/Pages/Testimonial/Edit.vue";
import Delete from "@/Pages/Testimonial/Delete.vue";
import DeleteBulk from "@/Pages/Testimonial/DeleteBulk.vue";
...
<template>
    <div class="flex justify-between">
        <div class="flex shrink-0 rounded overflow-hidden">
            <Create v-if="can(['testimonial create'])" />
            <DeleteBulk v-if="itemsSelected.length != 0 && can(['testimonial delete'])" :itemsSelected="itemsSelected"
                title="Items" />
        </div>
    ...
    <EasyDataTable class="mt-2" :rows-per-page="rowsPerPage" show-index alternating border-cell buttons-pagination
        ...
        <template #item-actions="item">
            <div class="flex w-fit rounded overflow-hidden">
                <Edit v-show="can(['testimonial update'])" :title="item.name" :item="item" @open="item = item" />
                <Delete v-show="can(['testimonial delete'])" :title="item.name" :item="item" @open="item = item" />
            </div>
        </template>
    </EasyDataTable>
</template>
