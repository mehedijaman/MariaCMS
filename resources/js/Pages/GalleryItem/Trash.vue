...
const items = reactive(props.gallery_items);
                    ...
                    <div class="flex justify-between">
                        <div class="flex shrink-0 rounded overflow-hidden">
                            <Link
                                ...
                                :href="route('gallery.items.index', {gallery: props.gallery.id})">
                            ...
                            <RestoreBulk v-if="itemsSelected.length != 0 && can(['gallery item delete'])"
                                :itemsSelected="itemsSelected" title="Items" />
                            <DeleteForceBulk v-if="itemsSelected.length != 0 && can(['gallery item delete'])"
                                :itemsSelected="itemsSelected" title="Items" />
                        </div>
                    ...
                    <EasyDataTable class="mt-2" :rows-per-page="rowsPerPage" show-index alternating border-cell
                        ...
                        <template #item-actions="item">
                            <div class="flex w-fit rounded overflow-hidden">
                                <Restore v-show="can(['gallery item update'])" :title="item.name" :item="item"
                                    @open="item = item" />
                                <DeleteForce v-show="can(['gallery item delete'])" :title="item.name" :item="item"
                                    @open="item = item" />
                            </div>
                        </template>
                    </EasyDataTable>
